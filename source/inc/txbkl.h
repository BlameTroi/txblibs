...
/*
 * a handle to a keyed linked list instance.
 */

typedef struct klcb hkl;
...
static const char *kl_err_duplicate_key    = "duplicate key";
static const char *kl_err_key_not_found    = "key not found";
static const char *kl_err_list_empty       = "list empty";
static const char *kl_err_next_at_tail     = "get next reached tail of list";
static const char *kl_err_previous_at_head = "get previos reached head of list";
static const char *kl_err_bad_update_key   = "update not positioned or bad key";
static const char *kl_err_bad_delete_key   = "delete not positioned or bad key";
...
kl_clone(
	...
	hkl *new_kl = kl_create(old_kl->compare_keys);
	if (kl_empty(old_kl))
		return new_kl;
	/* copy items */
	pkey key = NULL;
	...
	*value = NULL;
	...
	if (kl->position) {
		...
		*value = kl->position->value;
		...
		kl->error = kl_err_previous_at_head;
...
kl_update(
	...
	if (kl->position == NULL ||
		...
		kl->position = NULL;
		kl->error = kl_err_bad_update_key;
		return false;
	}
...
kl_delete(
	...
	if (kl->position == NULL ||
			kl->compare_keys(key, kl->position->key) != 0) {
		kl->position = NULL;
		kl->error = kl_err_bad_delete_key;
		return false;
	}
